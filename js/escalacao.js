...
window.addEventListener('DOMContentLoaded', () => {
    db.ref(`ligas/${ligaLogada}/usuarios/${userLogado}`).once('value').then(snapshot => {
        ...
        if(!dadosUsuario || !dadosUsuario.timeAtual || dadosUsuario.timeAtual === "Sem Clube") {
            alert("Você precisa assinar com um clube primeiro!");
        ...
        if(dadosUsuario.formacao) {
            document.getElementById('select-formacao').value = dadosUsuario.formacao;
        }

        // Corrige o array que o Firebase salvou pulando os 'null'
        ...
        // 1. DESENHA O CAMPO IMEDIATAMENTE (Evita a tela vazia)
        desenharCampinho(false);

        // 2. BUSCA OS JOGADORES DEPOIS
        carregarElenco(dadosUsuario.timeAtual);
...
function salvarEscalacao() {
    ...
    if(escaladosCount < 11) {
        if(!confirm(`Seu time tem apenas ${escaladosCount} jogadores em campo. Tem certeza que deseja salvar incompleto? O time será punido no jogo.`)) return;
    }

    const selectFormacao = document.getElementById('select-formacao');
    const formacaoAtual = selectFormacao ? selectFormacao.value : "4-4-2";
    const forcaTxt = document.getElementById('forca-time') ? document.getElementById('forca-time').innerText : "0";

    db.ref(`ligas/${ligaLogada}/usuarios/${userLogado}`).update({
        titulares: titulares,
        formacao: formacaoAtual,
        forcaAtual: parseInt(forcaTxt)
    }).then(() => {
        alert("✅ Tática e Escalação salvas com sucesso!");
