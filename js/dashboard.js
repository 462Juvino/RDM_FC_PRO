...
// 6. MOTOR DO JORNALZINHO
function gerarNoticia(meuTime) {
    const elem = document.getElementById('texto-noticia');
    if(!elem) return;

    const templatesNoticias = [
        `"Especulações fortíssimas indicam que a diretoria do ${meuTime} está preparando uma proposta milionária na próxima janela!"`,
        `"O campeonato mal começou e a imprensa já questiona se a tática escolhida pelo comandante do ${meuTime} vai funcionar na elite."`,
        `"Rumores: Um gigante europeu está de olho no artilheiro do campeonato. Será que teremos venda recorde?"`,
        `"Zebra à vista? Os analistas preveem uma rodada cheia de surpresas após os últimos treinos fechados."`,
        `"Torcida do ${meuTime} esgota ingressos para a próxima partida! Clima de caldeirão."`,
        `"Polêmica no vestiário! Jogador reclama de estar atuando fora de sua posição original."`
    ];

    const noticiaSorteada = templatesNoticias[Math.floor(Math.random() * templatesNoticias.length)];
...
// 7. MOTOR ROTATIVO DE ESTATÍSTICAS
let estadoEstatistica = 0;
function alternarEstatisticas() {
    ...
    if (estadoEstatistica === 0) {
        ...
        container.innerHTML = `
            <ul class="lista-info">
                <li><span class="nome-destaque">1. A definir</span> <span>0 Gols</span></li>
                <li><span class="nome-destaque">2. A definir</span> <span>0 Gols</span></li>
                <li><span class="nome-destaque">3. A definir</span> <span>0 Gols</span></li>
            </ul>`;
        estadoEstatistica = 1;
    } else {
        ...
        container.innerHTML = `
            <ul class="lista-info">
                <li><span class="nome-destaque">1. A definir</span> <span>0 Ast</span></li>
                <li><span class="nome-destaque">2. A definir</span> <span>0 Ast</span></li>
                <li><span class="nome-destaque">3. A definir</span> <span>0 Ast</span></li>
            </ul>`;
        estadoEstatistica = 0;
    }
