...
// 1. Puxa os dados do Campeonato
async function carregarTabela(timeIdBanco) {
    try {
        // Descobre em qual divisão o time do jogador está
        const snapTime = await db.ref(`banco_global_times/${timeIdBanco}`).once('value');
        meuTimeObj = snapTime.val();
        if (meuTimeObj && meuTimeObj.divisao) {
            divisaoAtual = meuTimeObj.divisao;
        }

        // Puxa o Calendário da Liga
        ...
        // Pega a tabela certa (Série A ou B)
        calendarioCompleto = divisaoAtual === "A" ? cal.serieA : cal.serieB;
        rodadaAtualSistema = cal.rodadaAtual || 1;
        document.getElementById('lbl-rodada-atual').innerText = rodadaAtualSistema;
