...
window.mudarRodadaTransmissao = function(novaRodada) {
    ...
    // Zera o estado do áudio ao mudar de jogo
...
function renderizarPartida() {
    ...
    if (topoRodada) {
        ...
        for (let r = 1; r <= rodadaSistema; r++) { selectHtml += `<option value="${r}" ${r === rodadaExibicao ? 'selected' : ''}>Rodada ${r}</option>`; }
        selectHtml += `</select>`; topoRodada.innerHTML = selectHtml;
    }
    ...
    if (jogoAoVivo) {
        ...
        if (jaTerminouDeVerdade) {
            ...
            if(narracao) {
                if (linhaArray.length > 0) {
                    ...
                    linhaArray.forEach(evento => {
                        ...
                        adicionarNarraçao(`${evento.minuto}'`, evento.texto.replace("GOOOL DO", `GOOOL DO ${escudoHTML}`), evento.cor);
                    });
                } else { narracao.innerHTML = `<div style="text-align:center; padding: 20px; color:#aaa;">O juiz apitou o fim de jogo! Partida sem lances de perigo ou melhores momentos (0x0).</div>`; }
            }
            ...
            if (horaAtual > (20 - 1)) horarioPermitido = true;
            if (horaAtual === (20 - 1) && minAtual >= 40) horarioPermitido = true;
            ...
            if (rodadaExibicao < rodadaSistema) {
                ...
                // Set audio sources initially if missing
                if (!canalTorcidaM.src || !canalTorcidaM.src.includes(jogoAoVivo.mandante)) {
...
function processarNarradorOficial() {
    ...
    narradorOficialOcupado = true; // Pausa a inserção de novos textos
    ...
    if (evento.tipo.includes("gol")) {
        if (evento.tipo.includes("mandante")) placarNarracaoM++; else placarNarracaoV++;
        ...
        if (audioLiberado) {
            if (evento.tipo.includes("mandante")) { canalTorcidaM.volume = 1.0; canalTorcidaV.volume = 0.0; }
            else { canalTorcidaM.volume = 0.0; canalTorcidaV.volume = 1.0; }

            canalEfeitos.src = 'sounds/gol_generico.mp3';
            canalEfeitos.play().catch(()=>{});

            setTimeout(() => {
                canalHino.src = getHino(escudoID);
                canalHino.volume = 0.4;
                canalHino.play().catch(()=>{});
            }, 1500);

            // Segura a narração oficial por 12 segundos enquanto toca o hino
            setTimeout(() => {
        ...
        // Lances comuns ou substituições
        if (audioLiberado) {
            if (evento.tipo.includes("mandante")) canalTorcidaM.volume = 0.8;
            else canalTorcidaV.volume = 0.8;
        }
        setTimeout(() => {
            atualizarTorcidasOficiais();
            narradorOficialOcupado = false;
...
function reproduzirLinhaDoTempo(linha, horaInicioTstamp, placarMFinal, placarVFinal) {
    const diferencaSegundos = Math.floor((Date.now() - horaInicioTstamp) / 1000);
    ...
    // Adiciona os lances novos na fila
    linha.forEach(evento => {
        let idEvento = `${evento.minuto}_${evento.tipo}`;
        if (evento.minuto <= minutoAtualJogo && !eventosJaTocados.has(idEvento)) {
            eventosJaTocados.add(idEvento);
            filaNarracaoOficial.push(evento);
        }
    });

    // Avisa o narrador para olhar a fila
    processarNarradorOficial();
}
